...
export default function ObjectLibrary() {
  ...
  const duplicateById = useWorld(s => s.duplicateById)
  ...
  return (
    <div className="panel" style={{ marginTop: 8, minWidth: 360 }}>
        ...
        <div key={g.id} style={{ marginTop: 6 }}>
          ...
          {blocks.filter(b => b.groupId === g.id).map(b => (
            <Row
              ...
              onClick={(multi) => multi ? toggleSelect(b.id) : selectOnly(b.id)}
              onDuplicate={() => duplicateById(b.id)}
              onDelete={() => removeById(b.id)}
            />
          ))}
        </div>
      ))}
      ...
      {grouped.ungrouped.length > 0 && (
        <div className="label" style={{ fontWeight: 600, marginTop: 8 }}>Ungrouped</div>
      )}
  ...
  return (
    <div
      ...
      style={{
        ...
        background: selected ? '#121722' : 'transparent',
        padding: '4px 6px',
        borderRadius: 8,
      }}
      onClick={(e) => onClick((e as any).shiftKey)}
    >
      <div className="row" style={{ gap: 8, alignItems: 'center' }}>
        <div style={{ width: 12, height: 12, borderRadius: 3, background: b.color }} />
        <div style={{ fontSize: 12 }}>
          {b.name || b.type}{' '}
          <span className="label">({b.size.join('×')})</span>
        </div>
      </div>
