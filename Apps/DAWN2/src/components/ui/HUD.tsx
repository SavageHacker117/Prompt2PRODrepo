...
const defaultLayout: Layout = {
  // Default: WORLD bottom-left, SCENE bottom-right
  docks: {
    left: [],
    right: ['anim', 'spawns', 'actors'],
    bottomL: ['world'],
    bottomR: ['scene'],
  },
  floats: [],
...
function normalizeV4(input: any): Layout {
  const safeArr = (v: any) => Array.isArray(v) ? v.filter((x: any) => VALID_WINS.includes(x)) : []
  ...
  const floats = floatsIn
    ...
    .map((f: any) => ({ id: f.id as WinId, x: +f.x || 60, y: +f.y || 60, w: Math.max(260, +f.w || 360) }))
...
function useLayout(): [Layout, (l: Layout | ((p: Layout) => Layout)) => void] {
  ...
  useEffect(() => { localStorage.setItem(LSK_V4, JSON.stringify(state)) }, [state])
  return [state, setState]
}

function useUiOpts(): [UiOpts, (p: Partial<UiOpts>) => void] {
  const [opts, setOpts] = useState<UiOpts>(() => {
    try { const raw = localStorage.getItem(OPTS_LSK); if (raw) return { ...DEFAULT_OPTS, ...JSON.parse(raw) } } catch {}
    return DEFAULT_OPTS
  })
  useEffect(() => { localStorage.setItem(OPTS_LSK, JSON.stringify(opts)) }, [opts])
...
/* ───────── tiny atoms ───────── */
function Tab({ id, active, onClick, onTear }:{
...
function DockArea({ ids, onDrop, children }:{
  ids: WinId[]; onDrop: (id: WinId) => void; children: React.ReactNode
}) {
  return (
    <div
      className="dock-area"
...
function Floating({ id, x, y, w, body, onClose, onMove, onResize }:{
...
/* ───────── WORLD pane (unchanged logic, small tidy) ───────── */
...
async function getStats(url: string) {
  ...
  let meshes = 0, skinned = 0, bones = 0, anims = gltf.animations?.length || 0, verts = 0
  ...
  return (
    <div className="panel">
      <div className="row wrap">
        ...
        <select className="input" value={palette.type} onChange={(e) => setPalette({ type: e.target.value as any })}>
        ...
        <input className="input" type="color" value={palette.color} onChange={(e) => setPalette({ color: e.target.value })} />
        ...
        <span style={{ flex: 1 }} />
        <button className="btn danger" onClick={() => setAskReset(true)}>Reset World</button>
      </div>
      ...
      {askReset && (
        <div className="opts-backdrop" onClick={() => setAskReset(false)}>
          <div className="confirm" onClick={e => e.stopPropagation()}>
            ...
            <div className="row">
              <button className="btn" onClick={() => setAskReset(false)}>Cancel</button>
              <button className="btn danger" onClick={() => { setAskReset(false); resetWorld() }}>Reset</button>
            </div>
          </div>
        </div>
      )}
...
function ActorsPane() {
  ...
  function baseName(u: string){ return (u.split('/').pop() || '').replace(/\.glb$/i,'').toLowerCase() }
  ...
  const scan = async () => {
    ...
    const guessModels: Model[] = guessed.filter(Boolean)
      .map(u => ({ name: u!.split('/').pop()!.replace(/\.glb$/i, ''), url: u!, source: 'guess' } as const))

    // 2) real manifest if present
    let manifest: Model[] = []
    try {
    ...
    const finalModels = Array.from(byName.values()).sort((a,b)=>a.name.localeCompare(b.name))
    setModels(finalModels); setBusy(false)
  }
  ...
  const showInfo = async (url: string) => {
    ...
    try { setPreview({ url, stats: await getStats(url) }) }
    catch { setPreview({ url, stats: null }) }
  }
  ...
  return (
    <div className="panel">
      <div className="panel" style={{ background: 'rgba(0,0,0,.25)', marginBottom: 8 }}>
        ...
        {preview && preview.stats === null && <div className="label" style={{ opacity: .8 }}>(failed to read stats)</div>}
      </div>
...
function AnimPane() {
  ...
  const [move, setMove] = useState<boolean>(() => !!(window as any).__engine?.movement?.enabled)
  ...
  return (
    <div className="panel">
      <div className="row" style={{ gap: 8, marginBottom: 8 }}>
        ...
        <span style={{ flex: 1 }} />
        <button className={`btn ${move ? 'ok' : ''}`} onClick={toggleMove} title="Enable WASD movement for the selected actor">
      ...
      <div className="row" style={{ gap: 12, marginTop: 6 }}>
        ...
        <label className="label"><input type="checkbox" onChange={() => (window as any).__animPanel?.wave?.toggle?.()} /> Wave</label>
      </div>
    </div>
  )
}
...
function OptionsModal({
  open, onClose, opts, setOpts, onResetLayout,
}: {
  open: boolean; onClose: () => void; opts: UiOpts; setOpts: (p: Partial<UiOpts>) => void; onResetLayout: () => void
}) {
  if (!open) return null
  return (
    <div className="opts-backdrop" onClick={onClose}>
      <div className="opts-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="opts-section">
          ...
          <label className="label"><input type="checkbox" checked={opts.lock} onChange={e => setOpts({ lock: e.target.checked })} /> Lock UI (panes, floats, splitter)</label>
          <label className="label"><input type="checkbox" checked={opts.compact} onChange={e => setOpts({ compact: e.target.checked })} /> Compact UI</label>
          <label className="label"><input type="checkbox" checked={opts.showConsole} onChange={e => setOpts({ showConsole: e.target.checked })} /> Show Debug Console</label>
          <div className="row" style={{ marginTop: 8, justifyContent: 'flex-end' }}>
            <button className="btn danger" onClick={onResetLayout} title="Restore default layout">Reset Dock Layout</button>
          </div>
        </div>
        ...
        <div className="opts-section">
          ...
          <div className="row" style={{ gap: 8, alignItems: 'center' }}>
            <div className="label" style={{ width: 100 }}>Pixel Ratio</div>
            <select className="input" value={String(opts.pixelRatio)} onChange={e => {
              const v = e.target.value === 'auto' ? 'auto' : (parseFloat(e.target.value) as 1 | 1.5 | 2)
          ...
          <div className="row" style={{ gap: 8, alignItems: 'center' }}>
            <label className="label"><input type="checkbox" checked={opts.shadows !== 'off'} onChange={e => setOpts({ shadows: e.target.checked ? 'pcfsoft' : 'off' })} /> Shadows</label>
            <select className="input" value={opts.shadows === 'off' ? 'pcfsoft' : opts.shadows} onChange={e => setOpts({ shadows: (e.target.value as any) })} disabled={opts.shadows === 'off'}>
              <option value="pcf">PCF</option>
              <option value="pcfsoft">PCF Soft</option>
            </select>
          </div>
          <div className="row" style={{ gap: 8, alignItems: 'center' }}>
            <div className="label" style={{ width: 100 }}>Exposure</div>
            <input className="input" type="range" min={0.2} max={2} step={0.01} value={opts.exposure} onChange={e => setOpts({ exposure: parseFloat(e.target.value) })} />
            <div className="label" style={{ width: 40, textAlign: 'right' }}>{opts.exposure.toFixed(2)}</div>
          </div>
        </div>

        <div className="label" style={{ opacity: .7, marginTop: 8 }}>Press Esc again to close.</div>
      </div>
    </div>
  )
}
...
export default function HUD() {
  ...
  useEffect(() => {
    ...
    if (root) root.classList.toggle('has-bottom', (layout.docks.bottomL.length + layout.docks.bottomR.length) > 0)
  }, [layout.docks.bottomL.length, layout.docks.bottomR.length])
  ...
  const onResizeDockStart = (e: React.MouseEvent) => {
    ...
    const startH = dockH
    const onMove = (ev: MouseEvent) => setDockH(Math.max(140, Math.min(520, startH + (startY - ev.clientY))))
  ...
  const moveToDock = (win: WinId, dest: DockKey) => {
    setLayout(prev => {
      ...
      ;(['left','right','bottomL','bottomR'] as DockKey[]).forEach(k => next.docks[k] = next.docks[k].filter(id => id !== win))
  ...
  const tearOff = (win: WinId) => {
    setLayout(prev => {
      const next: Layout = JSON.parse(JSON.stringify(prev))
      ;(['left','right','bottomL','bottomR'] as DockKey[]).forEach(k => next.docks[k] = next.docks[k].filter(id => id !== win))
      if (!next.floats.find(f => f.id === win)) next.floats.push({ id: win, x: 60, y: 60, w: 360 })
      return normalizeV4(next)
    })
  }
  ...
  const toggleBottomDock = () => {
    setLayout(prev => {
      ...
      if (anyBottom) {
        ...
        next.docks.right = Array.from(new Set([...next.docks.right, ...next.docks.bottomL, ...next.docks.bottomR]))
        next.docks.bottomL = []
        next.docks.bottomR = []
      } else {
        // create a default: WORLD left, SCENE right
        next.docks.bottomL = ['world']
        next.docks.bottomR = ['scene']
        // ensure they are not duplicated in other docks
        next.docks.left = next.docks.left.filter(id => id !== 'world' && id !== 'scene')
        next.docks.right = next.docks.right.filter(id => id !== 'world' && id !== 'scene')
      }
      return normalizeV4(next)
    })
  }
  ...
  }, [layout.docks.left, layout.docks.right, layout.docks.bottomL, layout.docks.bottomR])
  ...
  // Hotkeys: Esc for options; '9' Play/Edit (kept)
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') setShowOpts(o => !o)
      else if (e.code === 'Digit9' || e.key === '9') setMode(mode === 'play' ? 'edit' : 'play')
    }
    window.addEventListener('keydown', onKey)
    return () => window.removeEventListener('keydown', onKey)
  }, [mode, setMode])

  const hasBottom = layout.docks.bottomL.length > 0 || layout.docks.bottomR.length > 0

  return (
    <div className="hud">
      <div className="dock-resizer" onMouseDown={onResizeDockStart} />
      <button className="btn dock-add" title={hasBottom ? 'Remove second dock' : 'Add a second dock'} onClick={toggleBottomDock}>
        {hasBottom ? '− Dock' : '+ Dock'}
      </button>
          ...
          return (
            <DockArea key={side} ids={ids} onDrop={(id) => moveToDock(id, side)}>
              <div className="dock-tabbar">
                {ids.map((id) => (
                  <Tab key={id} id={id} active={active === id}
      ...
      {/* floating windows */}
      {layout.floats.map(f => (
        <Floating key={f.id} id={f.id} x={f.x} y={f.y} w={f.w} body={winBodies[f.id]}
          onClose={() => closeFloat(f.id)}
          onMove={(dx, dy) => { if (opts.lock) return; setLayout(prev => ({ ...prev, floats: prev.floats.map(ff => ff.id === f.id ? { ...ff, x: ff.x + dx, y: ff.y + dy } : ff) })) }}
          onResize={(dw) => { if (opts.lock) return; setLayout(prev => ({ ...prev, floats: prev.floats.map(ff => ff.id === f.id ? { ...ff, w: Math.max(260, ff.w + dw) } : ff) })) }}
        />
      ))}

      <div className="ghost-note">Edit: click to place, right-click to delete, R to rotate. Grid=1m • Press Esc for Options • 9 toggles Play/Edit</div>

      <OptionsModal open={showOpts} onClose={() => setShowOpts(false)} opts={opts} setOpts={setOpts} onResetLayout={resetDockLayout} />
    </div>
  )
}
