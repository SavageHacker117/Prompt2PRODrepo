...
export function DebugConsole() {
  ...
  // core commands (help, clear, etc.)
  useEffect(() => {
    core.extend(
      'help',
      () => {
        const names = Array.from(core.help.entries()).map(
          ([n, h]) => `${n} — ${h}`,
        )
    ...
    core.extend(
      ...
      (args) => {
        ...
        if (sub === 'scale') {
          ...
          const a: any =
            eng.activeActor || (eng.actors && Object.values<any>(eng.actors)[0])
          a?.setScale?.(v)
          return `scale ${isFinite(v) ? v.toFixed(2) : 'NaN'}`
        }

        if (sub === 'stats') {
          const root: any =
            (eng.activeActor && eng.activeActor.object) || null
    ...
    core.extend(
      ...
      (args) => {
        ...
        if (sub === 'helper') {
          ...
          if (s === 'on') {
            a.bones?.setOverlay?.(true)
            return 'bones helper: overlay on'
          } else if (s === 'off') {
            a.bones?.setOverlay?.(false)
            return 'bones helper: overlay off'
          } else if (s === 'toggle') {
            a.bones?.setOverlay?.()
            return 'bones helper: overlay toggled'
          }
  ...
  const onCopyLog = useCallback(async () => {
    const payload = log.join('\n')
    if (!payload) return
    try {
      await navigator.clipboard.writeText(payload)
      logBus.add('console: log copied to clipboard')
    } catch {
      try {
        ...
        logBus.add('console: log copied to clipboard (fallback)')
      } catch {
        logBus.add('console: failed to copy log')
      }
    }
  }, [log, logBus])
    ...
    <div className="console">
      ...
      <style>{`
        ...
        .help {
          ...
          max-height: 260px; /* taller log window */
          overflow: auto;
          font-family: "JetBrains Mono", "Fira Code", ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace;
        }

        body.hud-compact .console {
          top: 6px;
          font-size: 11px;
        }
      `}</style>
