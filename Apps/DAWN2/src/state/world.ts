...
const rid = () =>
  ((globalThis.crypto && 'randomUUID' in globalThis.crypto) ? crypto.randomUUID() : String(Date.now()+Math.random()))

const DEFAULT_PALETTE: Palette = { type: 'block', size: [1,1,1], color: '#ffffff' }

export const useWorld = create<State>()(persist((set, get) => {
  ...
  return {
    ...
    rotate: () => set({ rotY: (get().rotY + Math.PI/2) % (Math.PI*2) }),

    addBlock: (partial) => set({
      blocks: [...get().blocks, {
        id: rid(),
        type: partial?.type ?? get().palette.type,
        pos: (partial?.pos ?? get().hovered ?? [0,0,0]) as [number,number,number],
        size: (partial?.size ?? get().palette.size) as [number,number,number],
    ...
    removeAt: (pos) => set(() => {
      const blocks = get().blocks.filter(b => !(b.pos[0]===pos[0] && b.pos[1]===pos[1] && b.pos[2]===pos[2]))
    ...
    removeById: (id) => set(() => {
      ...
      return { blocks, selectedIds, groups: cleanupGroups(blocks, get().groups) }
    }),
    ...
    importJSON: (json) => {
      try {
        ...
        if (Array.isArray(data)) set({ blocks: data, groups: cleanupGroups(data, get().groups) })
        else if (data && Array.isArray(data.blocks)) {
          set({ blocks: data.blocks, groups: cleanupGroups(data.blocks, data.groups || []) })
        }
      } catch {}
    },

    exportJSON: () => JSON.stringify({ blocks: get().blocks, groups: get().groups }, null, 2),
    ...
    groupSelection: (name) => {
      ...
      const groupId = rid()
      const groupName = uniqueGroupName(name || `Group ${groups.length + 1}`, groups)
      const nextGroups = [...groups, { id: groupId, name: groupName }]
      const nextBlocks = get().blocks.map(b => ids.includes(b.id) ? { ...b, groupId } : b)
      set({ groups: nextGroups, blocks: nextBlocks })
    },

    ungroupSelection: () => {
      const ids = get().selectedIds
      const nextBlocks = get().blocks.map(b => ids.includes(b.id) ? { ...b, groupId: null } : b)
      set({ blocks: nextBlocks, groups: cleanupGroups(nextBlocks, get().groups) })
    },
