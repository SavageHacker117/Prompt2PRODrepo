// src/engine/perf/BudgetManager.ts
...
export type TrackableKind = "texture" | "mesh" | "env" | "instanced";
...
export class BudgetManager {
  ...
  track(t: Trackable) {
    ...
    if (t.kind === "mesh" || t.kind === "instanced") this.nodes += 1;
    this.trimIfNeeded();
  }
  ...
  private removeAt(i: number) {
    ...
    if (v.kind === "mesh" || v.kind === "instanced") this.nodes -= 1;
    try {
      v.dispose();
    } catch {
      /* ignore */
